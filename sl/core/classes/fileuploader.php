...
//        $this->controller=&$controller;
        $settings['ext'] = array('png', 'jpg','gif','pdf','doc','zip','tar.gz','gz','zip','rar','docx','txt' ,'xls' , 'xlsx' , 'xlsm' , 'xlam','odt','pptx');
        //$settings['limit'] = 22428800; //50mb
        if(!is_null($settings) && is_array($settings)){
            if(isset($settings['ext']) && is_array($settings['ext'])){
                $this->setAllowedExtensions($settings['ext']);
            }else{
                $this->setAllowedExtensions(array('pdf','doc','zip','tar.gz','gz'));
            }
            if(isset($settings['limit']) && is_integer($settings['limit'])){
                $this->setSizeLimit($settings['limit']);
            }else{
                $this->setSizeLimit(224288000);//5mb135224304
            }
        }
    }
    /**
...
class qqFileUploader {
    ...
    private $sizeLimit = 999999999999999;
    private $file;

    function __construct(array $allowedExtensions = array(), $sizeLimit = 999999999999999){
