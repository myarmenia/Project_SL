...
//            var_dump($for_solr);die;
            ...
            $offset = 0;
            $limit = 100;
            $_POST['content'] = $this->escapeSolrValue($_POST['content']);
...
//            var_dump($word);die;
            ...
            $search = '';
            foreach($final_word as $key=>$value){
                if(strpos($value,'\+')){
                    $value = str_replace('\+',' ',$value);
                    $value = $solr->phrase($value);
                }
                $search .= ($key != 0) ? ' AND attr_content:'.$value.'' : 'attr_content:'.$value.'';
            }
            $queries = array(
                $search,
            );
