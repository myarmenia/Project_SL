    ...
    <div id="example" class="k-content">
        ...
        <div style="text-align: right">
            <a class="k-button k-button-icontext k-grid-resetFilter"
                href="{{ route('simple_search_external_signs', ['locale' => app()->getLocale(), 'n' => 't']) }}">{{ __('content.new_search') }}</a>
            <a class="k-button k-button-icontext k-grid-resetFilter"
                href="{{ route('simple_search_external_signs', ['locale' => app()->getLocale(), 'n' => 'f']) }}">{{ __('content.change_search') }}</a>
        </div>
        <div id="grid"></div>
        <div class="details" id="table" data-tb-name="external_signs"></div>

    @section('js-include')
        <script>
            let ties = "{{ __('content.ties') }}"
            let parent_table_name = "{{ __('content.external_signs') }}"
        </script>
            ...
            $(document).ready(function() {
                ...
                var grid = $("#grid").kendoGrid({
                    ...
                    columns: [{
                            command: {
                                name: "aJoin",
                                text: "<img src='images/view.png' style='width: 30px;height: 30px;' title='`{{ __('content.view_ties') }}`' >",
                        ...
                        <?php if(auth()->user()->roles()->first()->hasPermissionTo('external_signs-edit') ) { ?> {
                            command: {
                                name: "aEdit",
                                text: "<img src='images/edit.png' style='width: 30px;height: 30px;' title='`{{ __('content.edit') }}`' >",
                        ...
                        },
                        {
                            command: {
                                name: "aWord",
                                text: "<img src='images/word.gif' style='width: 30px;height: 30px;' title='`{{ __('content.word') }}`' >",
                                click: openWord
                            },
                            width: "90px"
                        },
                        <?php if(auth()->user()->roles()->first()->hasPermissionTo('external_signs-delete')) { ?> {
                            command: {
                                name: "aDelete",
                                text: "<img src='images/delete.png' style='width: 30px;height: 30px;' title='`{{ __('content.delete') }}`' >",
