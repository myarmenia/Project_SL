...
<div id="example" class="k-content">
    ...
    <div style="text-align: right">
        ...
        href="{{ route('simple_search_man_bean_country', ['locale' => app()->getLocale(), 'n' => 't']) }}">{{ __('content.new_search') }}</a>
        <a class="k-button k-button-icontext k-grid-resetFilter"
        href="{{ route('simple_search_man_bean_country', ['locale' => app()->getLocale(), 'n' => 'f']) }}">{{ __('content.change_search') }}</a>
    </div>
...
@section('js-include')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            var grid = $("#grid").kendoGrid({
                ...
                columns: [
                    { command: {
                        ...
                        click: showDetailsManBeannCountry
                        },
                        width: "90px" },
                <?php if(auth()->user()->roles()->first()->hasPermissionTo('man_beann_country-edit')) { ?>
                { command: {
                    name:"aEdit",
                    text: "<i class='bi bi-pencil-square' style='width: 30px;height: 30px;font-size: 26px;' title='{{ __('content.edit') }}' ></i>",
                    click: editManBeanCountry
                    },
                    width: "90px" },
            <?php } ?>
            ...
            { field: "goal", width: "120px",title: `{{ __('content.purpose_visit') }}`  },
            { field: "country_ate",width: "130px", title: `{{ __('content.country_ate') }}` },
            { field: "entry_date",width: "145px", title: `{{ __('content.entry_date') }}`,  format: "{0:dd-MM-yyyy}",
                    filterable: {
                ui: setDatePicker ,
                        extra: true
            }
            },
            { field: "exit_date",width: "140px", title: `{{ __('content.exit_date') }}`,  format: "{0:dd-MM-yyyy}",
                    filterable: {
                ui: setDatePicker ,
                        extra: true
            }
            },
            { field: "region",width: "100px",title: `{{ __('content.region') }}` },
            { field: "locality", width: "220px",title: `{{ __('content.locality') }}` },
            { command: {
                name:"aWord",
                ...
                width: "90px" },
            <?php if(auth()->user()->roles()->first()->hasPermissionTo('man_beann_country-delete')) { ?>
                { command: {
                    name:"aDelete",
...
//                    content:`/${lang}/open/weaponJoins/`
        ...
        $('#addNewManBeannCountry').click(function(e){
            ...
            $.ajax({
                url:`/${lang}/add/man_beann_country/`+tb_name,
                dataType : 'html',
        ...
        function tableDelete<?php echo $_SESSION['counter']; ?>(e) {
            ...
            if(confDel){
                $.ajax({
                    url: `/${lang}/admin/optimization_man_bean_country/`,
                    type: 'post',
                    data: { 'id' : dataItem.id } ,
                    success: function(data){
        ...
        function showDetailsManBeannCountry(e) {
            e.preventDefault();
            var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
            $('.k-window-title').html(`{{ __('content.ties_man_bean_country') }}`+dataItem.id);
            wnd.refresh({ url: `/${lang}/open/manBeannCountryJoins/`+dataItem.id });
            wnd.center().open();
        }
        ...
        function openWord(e) {
            e.preventDefault();
            var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
            window.open(`/${lang}/word/man_bean_country/`+dataItem.id, '_blank' );
        }

        function editManBeanCountry(e){
            var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
            $.ajax({
                url: `/${lang}/add/man_beann_country/edit/`+dataItem.id,
                dataType: 'html',
                success: function(data){
                    addItem(data,`{{ __('content.man_bean_country') }}`);
                },
                faild: function(data){
                    alert(`{{ __('content.err') }}` );
                }
