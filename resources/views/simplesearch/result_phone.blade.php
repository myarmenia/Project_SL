...
<div id="example" class="k-content">
    <div style="width: 70%; text-align: left">
        <?php
        $keyArray = array("number", "nature_character", "character_man_idName", "character_organization", "character_organization_idName", "more_data", "content");
...
@section('js-include')
    <script>
        ...
        $(document).ready(function () {
            ...
            { field: "number", width: "220px",title: `{{ __('content.phone_number') }}` },

            { field: "character_man",width: "200px", title: `{{ __('content.nature_character_man') }}` },
            { field: "character_organization",width: "260px", title: `{{ __('content.nature_character_organization') }}` },
            { field: "more_data",width: "260px", title: `{{ __('content.additional_data') }}` },
            { command: {
                name:"aWord",
...
//                    content:`/{{ app()->getLocale() }}/open/weaponJoins/`
        ...
        function tableDelete<?php echo $_SESSION['counter']; ?>(e) {
            ...
            if(confDel){
                $.ajax({
                    url: `/{{ app()->getLocale() }}/admin/optimization_phone/`,
                    type: 'post',
                    data: { 'id' : dataItem.id } ,
                    success: function(data){
        ...
        function showDetailsPhone(e) {
            ...
            wnd.refresh({ url: `/{{ app()->getLocale() }}/open/phoneJoins/`+dataItem.id });
            wnd.center().open();
        }
