    ...
    <div class="inContent">
        <form id="bibliographyForm" action="/{{ app()->getLocale() }}/simplesearch/result_bibliography" method="post">
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" name="from_agency_id_type" id="searchBiblFromAgencyNameType"
                    value="<?php echo $search_params['from_agency_id_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
                <label for="searchBiblFromAgencyName">{{ __('content.organ') }}</label>
                <input type="button" dataName="searchBiblFromAgencyName" dataId="searchBiblFromAgencyNameId"
                    dataTableName="fancy/agency" class="addMore k-icon k-i-plus my-plus-class" data-bs-toggle="modal"
                    data-bs-target="#fullscreenModal" data-fieldname="name" data-table-name="agency" />
                <input type="text" name="from_agency_name" id="searchBiblFromAgencyName"
                    class="oneInputSaveEnter fetch_input_title get_datalist" firstItem="1"
                    dataInputId="searchBiblFromAgencyNameId" dataTableName="agency" list="agency" />
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="category_id_type" id="searchBiblDocCatTitleType"
                    value="<?php echo $search_params['category_id_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
                <label for="searchBiblDocCatTitle">{{ __('content.document_category') }}</label>
                <input type="button" dataName="searchBiblDocCatTitle" dataId="searchBiblDocCatTitleId"
                    dataTableName="fancy/doc_category" class="addMore k-icon k-i-plus my-plus-class"
                    data-bs-toggle="modal" data-bs-target="#fullscreenModal" data-fieldname="name"
                    data-table-name="doc_category" />
                <input type="text" name="category_title" id="searchBiblDocCatTitle"
                    class="oneInputSaveEnter fetch_input_title get_datalist" dataInputId="searchBiblDocCatTitleId"
                    dataTableName="doc_category" list="doc_category" />
                @if (isset($search_params['category_id_type']) && $search_params['category_id_type'] == 'OR')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblDocCatTitleOp">{{ __('content.or') }}</span>
                @elseif (isset($search_params['category_id_type']) && $search_params['category_id_type'] == 'AND')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblDocCatTitleOp">{{ __('content.and') }}</span>
                @elseif (isset($search_params['category_id_type']) && $search_params['category_id_type'] == 'NOT')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblDocCatTitleOp">{{ __('content.not_equal') }}</span>
                @endif
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="access_level_id_type" id="searchBiblAccessLevelNameType"
                    value="<?php echo $search_params['access_level_id_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
                <label for="searchBiblAccessLevelName">{{ __('content.access_level') }}</label>
                <input type="button" dataName="searchBiblAccessLevelName" dataId="searchBiblAccessLevelNameId"
                    dataTableName="fancy/access_level" class="addMore k-icon k-i-plus my-plus-class"
                    data-bs-toggle="modal" data-bs-target="#fullscreenModal" data-fieldname="name"
                    data-table-name="access_level" />
                <input type="text" name="access_level_name" id="searchBiblAccessLevelName"
                    class="oneInputSaveEnter fetch_input_title get_datalist" dataInputId="searchBiblAccessLevelNameId"
                    dataTableName="access_level" list="access_level" />
                @if (isset($search_params['access_level_id_type']) && $search_params['access_level_id_type'] == 'OR')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblAccessLevelNameOp">{{ __('content.or') }}</span>
                @elseif (isset($search_params['access_level_id_type']) && $search_params['access_level_id_type'] == 'AND')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblAccessLevelNameOp">{{ __('content.and') }}</span>
                @elseif (isset($search_params['access_level_id_type']) && $search_params['access_level_id_type'] == 'NOT')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblAccessLevelNameOp">{{ __('content.not_equal') }}</span>
                @endif
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="worker_name_type" id="searchBiblWorkerNameType"
                    value="?= $search_params['worker_name_type'] ?>">
            </div>
            <?php } ?>
            <div class="forForm">
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="source_agency_id_type" id="searchBiblSourceAgencyNameType"
                    value="<?php echo $search_params['source_agency_id_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
                <label for="searchBiblSourceAgencyName">{{ __('content.source_agency') }}</label>
                <input type="button" dataName="searchBiblSourceAgencyName" dataId="searchBiblSourceAgencyNameId"
                    dataTableName="fancy/agency" class="addMore k-icon k-i-plus my-plus-class" data-bs-toggle="modal"
                    data-bs-target="#fullscreenModal" data-fieldname="name" data-table-name="agency" />
                <input type="text" name="source_agency_name" id="searchBiblSourceAgencyName"
                    class="oneInputSaveEnter fetch_input_title get_datalist" dataInputId="searchBiblSourceAgencyNameId"
                    dataTableName="agency" list="agency_one" />
                @if (isset($search_params['source_agency_id_type']) && $search_params['source_agency_id_type'] == 'OR')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblSourceAgencyOp">{{ __('content.or') }}</span>
                @elseif (isset($search_params['source_agency_id_type']) && $search_params['source_agency_id_type'] == 'AND')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblSourceAgencyOp">{{ __('content.and') }}</span>
                @elseif (isset($search_params['source_agency_id_type']) && $search_params['source_agency_id_type'] == 'NOT')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblSourceAgencyOp">{{ __('content.not_equal') }}</span>
                @endif
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="source_address_type" id="searchBiblSourceAddressType"
                    value="<?php echo $search_params['source_address_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
            ...
            <div style="width: 100%;text-align: right;top:14px;position: relative;">
                ...
                <input type="hidden" class="oneInputSaveEnter" readonly="readonly">
                <input type="hidden" name="related_year_type" id="searchBiblRelatedYearType"
                    value="<?php echo $search_params['related_year_type']; ?>">
            </div>
            <?php } ?>
            <div class="forForm">
            ...
            <div class="forForm">
                ...
                <input type="button" dataName="searchBiblCountry" dataId="searchBiblCountryId"
                ...
                <input type="text" name="country" id="searchBiblCountry"
                    class="oneInputSaveEnter fetch_input_title get_datalist" dataInputId="searchBiblCountryId"
                    dataTableName="country" list="country" />
                @if (isset($search_params['country_id_type']) && $search_params['country_id_type'] == 'OR')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblCountryOp">{{ __('content.or') }}</span>
                @elseif (isset($search_params['country_id_type']) && $search_params['country_id_type'] == 'AND')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblCountryOp">{{ __('content.and') }}</span>
                @elseif (isset($search_params['country_id_type']) && $search_params['country_id_type'] == 'NOT')
                    <span style="width: 30px;;position: absolute;margin-left: -570px;"
                        id="searchBiblCountryOp">{{ __('content.not_equal') }}</span>
                @endif
...
@section('js-include')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#userId').live('change', function(e) {
                ...
                if (user_id) {
                    ...
                    $('#searchBiblUser').append('<li id="listItemsearchBiblUser' + user_id + '">' +
                        '<div class="item">' +
                        '<span>' + text + '</span>' +
                        '<a user_id="' + user_id + '" text="' + text +
                        '" class="deleteMultiSearchOption">x</a>' +
                        '</div>' +
            ...
            $('#searchBiblFromAgencyNameId').val(
                `{{ $search_params['from_agency_id'][sizeof($search_params['from_agency_id']) - 1] }}`);
            $('#searchBiblAccessLevelName').val(`{{ html_entity_decode($search_params['access_level_name']) }}`);
            $('#searchBiblAccessLevelNameId').val(
                `{{ $search_params['access_level_id'][sizeof($search_params['access_level_id']) - 1] }}`);
            $('#searchBiblDocCatTitle').val(`{{ html_entity_decode($search_params['category_title']) }}`);
            $('#searchBiblDocCatTitleId').val(
                `{{ $search_params['category_id'][sizeof($search_params['category_id']) - 1] }}`);
            $('#searchRegNumber').val(
                `{{ html_entity_decode($search_params['reg_number'][sizeof($search_params['reg_number']) - 1]) }}`
                );
            $('#searchBiblWorkerName').val(
                `{{ html_entity_decode($search_params['worker_name'][sizeof($search_params['worker_name']) - 1]) }}`
                );
            $('#searchBiblSourceAddress').val(
                `{{ html_entity_decode($search_params['source_address'][sizeof($search_params['source_address']) - 1]) }}`
                );
            $('#searchBiblShortDesc').val(
                `{{ html_entity_decode($search_params['short_desc'][sizeof($search_params['short_desc']) - 1]) }}`
                );
            $('#searchBiblRelatedYear').val(
                `{{ html_entity_decode($search_params['related_year'][sizeof($search_params['related_year']) - 1]) }}`
                );
            $('#searchBiblSource').val(
                `{{ html_entity_decode($search_params['source'][sizeof($search_params['source']) - 1]) }}`);
            $('#searchBiblTheme').val(
                `{{ html_entity_decode($search_params['theme'][sizeof($search_params['theme']) - 1]) }}`);
            $('#searchBiblTitle').val(
                `{{ html_entity_decode($search_params['title'][sizeof($search_params['title']) - 1]) }}`);
            ...
            $('#searchBiblCountryId').val(
                `{{ $search_params['country_id'][sizeof($search_params['country_id']) - 1] }}`);
            <?php } ?>
        });
