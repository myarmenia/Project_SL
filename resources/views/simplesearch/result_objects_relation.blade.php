...
@section('content-include')
    ...
    <div id="example" class="k-content">
        <div style="width: 70%; text-align: left">
            <?php
            $keyArray = ['character_link', 'relation_type_idName', 'content'];
        ...
        <div style="text-align: right">
            <a class="k-button k-button-icontext k-grid-resetFilter"
                href="{{ route('simple_search_objects_relation', ['locale' => app()->getLocale(), 'n' => 't']) }}">{{ __('content.new_search') }}</a>
            <a class="k-button k-button-icontext k-grid-resetFilter"
                href="{{ route('simple_search_objects_relation', ['locale' => app()->getLocale(), 'n' => 'f']) }}">{{ __('content.change_search') }}</a>
        </div>
        <div id="grid"></div>
    ...
    @section('js-include')
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                var grid = $("#grid").kendoGrid({
                    ...
                    columns: [{
                        ...
                        <?php if(auth()->user()->roles()->first()->hasPermissionTo('objects_relation-edit') ) { ?> {
                            command: {
                                name: "aEdit",
                        ...
                        },
                        {
                            ...
                            width: "130px",
                            title: `{{ __('content.character_link') }}`
                        },
                        {
                            field: "first_object_id",
                            width: "100px",
                            title: `{{ __('content.first') }}`
                        },
                        {
                            field: "second_object_id",
                            width: "100px",
                            title: `{{ __('content.second') }}`
                        },
                        {
                            field: "first_object_type",
                            width: "160px",
                            title: `{{ __('content.first_object_type') }}`
                        },
                        {
                            field: "second_obejct_type",
                            width: "160px",
                        ...
                        //     width: "90px"
                        // },
                        <?php if(auth()->user()->roles()->first()->hasPermissionTo('objects_relation-delete')) { ?> {
                            command: {
                                name: "aDelete",
                ...
                $('#addNewObjectsRelation').click(function(e) {
                    ...
                    $.ajax({
                        url: `/${lang}/add/objects_relation/` + tb_name,
                        dataType: 'html',
            ...
            //     e.preventDefault();
            //     var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
            //     window.open(`/${lang}/word/objects_relation/`+dataItem.id, '_blank' );
            // }

            function editObject(e) {
                var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
                $.ajax({
                    url: `/${lang}/add/object/edit/` + dataItem.id,
                    dataType: 'html',
                    success: function(data) {
                        addItem(data, `{{ __('content.relationship_objects') }}`);
                    },
                    faild: function(data) {
                        alert(`{{ __('content.err') }}`);
                    }
