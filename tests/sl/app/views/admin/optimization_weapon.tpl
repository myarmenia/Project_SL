...
<div id="example" class="k-content" style="height: 600px;">
    ...
    <div id="grid_optimization_weapon"></div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var grid = $("#grid_optimization_weapon").kendoGrid({
...
//                    content:'<?php echo ROOT; ?>open/weaponJoins/'
    ...
    function tableDelete(e) {
        ...
        if(confDel){
            $.ajax({
                ...
                data: { 'id' : dataItem.id } ,
                success: function(data){
                    $("#grid_optimization_weapon").data("kendoGrid").dataSource.remove(dataItem);
                },
                faild: function(data){
    ...
    function editWeapon(e) {
        ...
        var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
        $.ajax({
            url: '<?php echo ROOT?>add/weapon/'+dataItem.bibliography_id+'/'+dataItem.id,
            dataType: 'html',
            success: function(data){
